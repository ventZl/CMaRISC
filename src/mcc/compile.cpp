...
CC::Assembly* CC::FunctionArgumentAllocator::getAddress(REGISTER return_reg, CC::Context* ctxt, CC::Variable* var) {
	Assembly * code = ctxt->createAssembly(return_reg);
	REGISTER a_reg = ctxt->getRegisters()->alloc(REGISTER_GENERIC);
	code->setReturnRegister(a_reg);
	code->appendCode(opcode("MOV", a_reg, this->offset));	// lame, very lame
	return code;
}
